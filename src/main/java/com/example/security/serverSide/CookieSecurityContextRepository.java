...
/**
 * The SecurityContext mainly represents the persisted session.
 * It contains an Authentication which in the context of a web application encapsulates
 * the information of the authenticated user.
 * The default implementation of the SecurityContextRepository stores the SecurityContext in the HttpSession.
 * To change this behaviour we have to provide our own SecurityContextRepository implementation.
 */
@Component
public class CookieSecurityContextRepository implements SecurityContextRepository {
    ...
    /**
     * The SaveToCookieResponseWrapper gets the UserInfo from the SecurityContext
     * and puts it into a SignedUserInfoCookie.
     * The SignedUserInfoCookie is an extension of javax.servlet.http.Cookie that handles the serialization
     * and deserialization of the UserInfo into/from the cookie value.
     */
    private class SaveToCookieResponseWrapper extends SaveContextOnUpdateOrErrorResponseWrapper {
        ...
        protected void saveContext(SecurityContext securityContext) {
            ...
            if (ANONYMOUS_USER.equals(authentication.getPrincipal())) {
                LOG.debug("Anonymous Company SecurityContext, skip saveContext");
                return;
            }
